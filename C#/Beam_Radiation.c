...
// Compile codes below in order

// module purge
// module load intel impi imkl
// gcc -Wall -std=c99 -o  Assignment5_Cathal_Lee_output Assignment5_Cathal_Lee.c -lmkl -liomp5 -lm
// ./Assignment5_Cathal_Lee_output
...
int main(void) {
  ...
  // Read coefficients from coefficients.txt
  ...
  if (!init_band_mat(&bmat, nbands_low, nbands_up, ncols)) {
    printf("Band matrix initialisation failed\n");
    free(F);
    free(f);
    free(f_new);
    free(RHS);
  ...
  for (long i = 0; i < Nx; i++) {
    ...
    for (long j = 0; j < Nv; j++) {
      ...
      // Initialise variables that depends on upwind direction
  ...
  for (long n = 0; n < Imin; n++) {
    ...
    for (long k = 0; k < bmat.nbrows_inv * bmat.ncol; k++) {
      bmat.array_inv[k] = 0.0;
    }
  ...
  for (long j = 0; j < Nv; j++) {
    for (long i = 0; i < Nx; i++) {
      long idx = i * Nv + j;
      // Clip the floating point implicit solver error (negligible)
      if (fabs(f[idx]) < 1e-14){
      f[idx] = 0.0;}
      fprintf(out, "%g\n", f[idx]);
    }
